...
//This is where user clicks are taken and overall graphics are implemented.
...
//		shape = s;
	...
	/**
	 * startFourier() - This will start the fourier transformation from scratch.
	...
	/**
	 * paintComponent() - draws all shapes and phases for drawing.
	 * Note that positive / negative y coordinates are inverted due to Java's graphics.
	 */
...
//				g.drawLine(fourierPoints[0].get(i).intValue(), fourierPoints[1].get(i).intValue(), 640, 386); //Filled-in line ticker.
	...
	/**
	 * mouseClicked() - This is for drawing the shape.
	 * Note that these coordinates must be polar, as we have these drawings stemming from the origin and
	 * 		therefore can't produce any other type of function without moving the origin within the system.
	 */
	public void mouseClicked(MouseEvent e) { //Note that positive / negative Y are inverted due to Java graphics.
		if (!connected) {
